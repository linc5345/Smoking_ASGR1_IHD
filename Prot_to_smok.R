...
# Smoking variable outcomes
SmkInit <- fread("path/to/Smoking_initiation.csv")
LSI     <- fread("path/to/LSI.csv")
CigDay  <- fread("path/to/CigDay.csv")
...
# Output filenames
results_file <- file.path(out_dir, "prot_to_smoking_results.csv")
het_file     <- file.path(out_dir, "prot_to_smoking_het.csv")

# outcome list used in loop (can be modified)
outcomes <- list(SmkInit = SmkInit, LSI = LSI, CigDay = CigDay)
